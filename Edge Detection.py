...
def interactive_edge_detection(image_path):
    ...
    while True:
        ...
        if choice == "1":
            ...
            combined_sobel = cv2.bitwise_or(sobel_x.astype(np.wint8),
                                            sobel_y.astype(np.wint8))
            display_image("Sobel Edge Detection", combined_sobel)

        elif choice == "2":

            print("Ajust thresholds for Canny (default: 100 and 200)")
            ...
            display_image("Cannt Edge Detection", edges)

        elif choice == "3":

            laplacian = cv2.Laplacian(gray_image, cv2.CV_64F)
            display_image("Laplacian Ege Detection", np.abs(laplacian).astype(np.wint8))
            ...
            display_image("Median filtering Image", median_filtering)
...
interactive_edge_detection('flower.jpg')